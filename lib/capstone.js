 ...
 * - `code` : The code `Buffer` being skipped
 * - `offset` : Offset within the original code buffer
 ...
 * disassembling.
 *
 * *NOTE: The `code` buffer is given a size of 1 byte by default. If you want to
 * access more data in the buffer, you can reinterpret its size:*
 *
 * ```javascript
 * var cs = new Cs(capstone.ARCH_X86, capstone.MODE_64);
 *
 * // Enable SKIPDATA mode with a callback
 * cs.skipdata = new capstone.CsSkipdata(".db", function (code, offset, user) {
 *     // Reinterpret `code` buffer to be 4 bytes long, starting at offset `0`
 *     code = code.reinterpret(4, 0);
 *
 *     console.log(
...
exports.CsSkipdata = function (mnemonic, callback, user_data) {
    ...
    skipdata.mnemonic = mnemonic;
    skipdata.callback = SkipdataCbType.toPointer(function (code, offset, user) {
        return callback(code, offset, user.readObject(0));
    });
